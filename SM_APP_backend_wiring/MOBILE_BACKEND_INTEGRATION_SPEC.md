...
- **Client dedupe:** Before calling the endpoint the mobile app now drops duplicate queue rows that share the same `mobile_uuid`, so backend duplicate responses should primarily represent replays detected server-side (e.g., job clocks or analytics replays).
...
- **Queue corruption handling (`DL-008`):** When Drift cannot decode a queued payload the client now copies the row into `corrupt_queue_entries` (schema v4) with the raw JSON/error, deletes the bad queue row, and emits a `QueueAlert` that surfaces in the UI so technicians contact support before exiting the job. Backend ops should expect these archived rows during troubleshooting and may request the payload via support scripts or future telemetry to reconcile missing punches.
